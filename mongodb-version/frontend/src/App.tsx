          ...
          {userMetadata ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: userMetadata.role === 'Admin' ? 'var(--accent-gold)' : 'var(--accent-mint)' }}></div>
      ...
      <header className="navbar">
        ...
        <div className="nav-actions">
          {userMetadata?.role === 'Admin' && (
            ...
            >
              <Shield size={12} />
              <span>{isAdminRoute ? 'Booking' : 'Admin'}</span>
            </button>
          )}
          {userMetadata && (
                        ...
                        {resources.map((facility) => (
                          ...
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                              <div>
                                <strong style={{ fontSize: '1.1rem', color: 'var(--text-primary)' }}>{facility.name}</strong>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                              ...
                              <div style={{ textAlign: 'right' }}>
                                Hourly Rate: <strong style={{ color: 'var(--text-primary)', fontSize: '1.1rem' }}>${(facility.price_per_hour || 50.0).toFixed(2)} / hr</strong>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <table style={{ fontSize: '0.8rem' }}>
                        ...
                        <tbody>
                          {adminBookings.map((bk) => (
                            <tr key={bk.id} className="admin-booking-row" style={{ opacity: bk.status === 'Cancelled' ? 0.6 : 1 }}>
                              <td style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{bk.id}</td>
                              <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{bk.user_name}</td>
                              <td style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{bk.user_phone}</td>
                              <td style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{bk.resource_name}</td>
                              <td style={{ fontWeight: 700, color: 'var(--accent-mint)' }}>{bk.date}</td>
                              <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{bk.slot_time}</td>
                              <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>${(bk.total_price || 0.0).toFixed(2)}</td>
                              <td style={{ color: 'var(--state-warning)', fontWeight: 700 }}>-${(bk.discount_applied || 0.0).toFixed(2)}</td>
                              <td>
                                <span style={{
                                  display: 'inline-block',
                ...
                <div className="card">
                  ...
                  {searchResultBookings.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginTop: '1.5rem' }}>
                      <h4 style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-primary)', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                        ...
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                            <div>
                              ...
                              <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: '0.15rem' }}>{bk.resource_name}</h3>
                            </div>
                            <span style={{
                              display: 'inline-block',
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '1rem', fontSize: '0.85rem' }}>
                            <div>
                              <span style={{ color: 'var(--text-primary)', display: 'block', fontSize: '0.75rem', fontWeight: 700 }}>BOOKED DATE</span>
                              <strong style={{ color: 'var(--text-primary)' }}>{bk.date}</strong>
                            </div>
                            <div>
                              <span style={{ color: 'var(--text-primary)', display: 'block', fontSize: '0.75rem', fontWeight: 700 }}>SCHEDULE TIME</span>
                              <strong style={{ color: 'var(--text-primary)' }}>{bk.slot_time}</strong>
                            </div>
                            <div>
                              <span style={{ color: 'var(--text-primary)', display: 'block', fontSize: '0.75rem', fontWeight: 700 }}>TOTAL CHARGED</span>
                              <strong style={{ color: 'var(--text-primary)' }}>${(bk.total_price || 0).toFixed(2)}</strong>
                            </div>
                            {bk.status !== 'Cancelled' && (
                      ...
                      <div className="booking-statement-ticket" style={{ marginBottom: '2rem', textAlign: 'left' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', fontSize: '0.85rem' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span className="booking-ticket-label">Customer</span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span className="booking-ticket-label">Date</span>
                            <strong style={{ color: 'var(--text-primary)' }}>{bookingSuccess.bookings[0]?.date}</strong>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span className="booking-ticket-label">Time Slots</span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px dashed var(--border-color)', paddingTop: '0.6rem', marginTop: '0.25rem' }}>
                            <span className="booking-ticket-label">Total Fee Charged</span>
